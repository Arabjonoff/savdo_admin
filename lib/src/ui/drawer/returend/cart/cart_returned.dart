...
class CardReturnedScreen extends StatefulWidget {
  final bool isNavigate;
  final dynamic data;
  const CardReturnedScreen({super.key, this.isNavigate=true, this.data});

  @override
  State<CardReturnedScreen> createState() => _CardReturnedScreenState();
}

class _CardReturnedScreenState extends State<CardReturnedScreen> {
  @override
  void initState() {
                            ...
                            return Slidable(
                              endActionPane: ActionPane(
                                ...
                                children: [
                                  SlidableAction(
                                    onPressed: (i){
                                      cartOutcomeBloc.deleteCartOutcome(data[index].id,widget.data,data[index].idSkl2,widget.data,context,true);
                                    },
                              ...
                              child: GestureDetector(
                                onTap: (){
                                  BottomDialog.showScreenDialog(context,UpdateOutcomeWidgetDialog(data: data[index], price: data[index].snarhi!=0?data[index].snarhi:data[index].snarhiS, priceUsd: data[index].frS.toInt(), typeName: data[index].vz,ndocId: widget.data, id: data[index].id,isReturned: true, count: data[index].soni,));
                                },
                                child: Container(
                    ...
                    Row(
                      children: [
                        Expanded(child: ButtonWidget(onTap: ()async{
                          // await repository.lockOutcome(widget.data,3);
